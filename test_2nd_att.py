...
with open('./output_train_diff_att/%s/setting.txt' % args_.experiment_name) as f:
    args = json.load(f)
...
# initialization
ckpt_dir = './output_train_diff_att/%s/checkpoints' % experiment_name
...
try:
    ...
    for idx, batch in enumerate(te_data):
        ...
        save_dir = './output_train_diff_att/%s/%s' % (experiment_name, save_folder)
